...
def RWR_target(target_edges, groundtruth, tar_neg_edges, alpha,emb_size,length_walk,num_walks,window_size,num_iters):
    all_nodes = target_edges[:, :].flatten()
    all_nodes = np.unique(all_nodes)

    node_num = len(all_nodes)
    print("----------------------当前网络target，节点个数：", node_num)

    np.random.shuffle(target_edges)

    online_len = target_edges.shape[0]
    train_size = math.floor(0.8 * online_len)
    val_size = math.floor(0.1 * online_len)
    # test_size = math.floor(0.1 * online_len)
    train_online_edges = target_edges[:, :]
    # val_online_edges = online_edges[train_size:val_size+train_size,:]
    test_size_edges = target_edges[val_size + train_size:, :]
    ...
    w2v = deepwalk.train(workers=4, is_loadmodel=False, is_loaddata=False, node_type="target")

    pos_test = test_size_edges
    neg_test = tar_neg_edges
